...
| `nagHideSelectors` | `string[]` | CSS selectors for an "open our app" nag banner to hide, if the SPA has one. Mirrors `SpaConfig.kt` on the Android side. Empty by default — arkware won't guess selectors for you. Passed through to both `arkware android build` and `arkware linux build`. |
...
| Field | Type | Notes |
|---|---|---|
| `enabled` | `boolean` | Must be `true` for `arkware android build`. |
...
controls what `build` writes. Actual APK builds happen in CI on
`main`.
...
| Field | Type | Notes |
|---|---|---|
| `enabled` | `boolean` | Must be `true` for `arkware linux build`. **Defaults to `false`** — opt-in, since it needs a heavier local toolchain than `android build`. See [`linux-shell.md`](./linux-shell.md#why-platformslinuxenabled-defaults-to-false). |
...
|---|---|
| `arkware android build` | `android.enabled`, `android.flavor` |
